...
impl CommandHelper {
    ...
    async fn auto_init_git_worktree_workspace(
        ...
        let workspace_store = SimpleWorkspaceStore::load(main_workspace.repo_path())?;
        if git_paths.worktree_root.join(".jj").is_dir() {
            repair_jj_repo_link(&git_paths.worktree_root, main_workspace.repo_path())?;
            let workspace = self.load_workspace_at(&git_paths.worktree_root, self.settings())?;
            if workspace.repo_path() == main_workspace.repo_path()
                && repo
                    .view()
        ...
        let workspace_name = unique_workspace_name(&repo, &git_paths.worktree_root)?;
...
",
    ...
    fn git_worktree_repo_context(&self) -> Result<Option<GitWorktreeRepoContext>, CommandError> {
        ...
        // Confirm the recorded main workspace root really is a Git workdir of
        // this repo before assuming its worktrees are ours to manage.
        if jj_lib::git::get_git_backend(self.repo().store()).is_ok_and(|backend| {
            backend
                .open_git_repo_at_workdir(&main_workspace_root)
                .is_err()
        }) {
            return Ok(None);
        }
    ...
    pub fn ensure_current_workspace_git_worktree(&self, ui: &Ui) -> Result<bool, CommandError> {
        ...
        if !git_head_resolves(&git_repo) {
            return Ok(false);
        }
        jj_lib::git::create_worktree_at_git_head(
            self.repo().store(),
            ctx.git_settings.to_subprocess_options(),
            self.workspace_root(),
        )?;
        writeln!(
            ui.status(),
    ...
    pub async fn forget_removed_git_worktrees(&mut self, _ui: &Ui) -> Result<(), CommandError> {
        ...
        let git_repo = jj_lib::git::get_git_repo(self.repo().store())?;
        let live_worktree_paths = git_worktree_paths(&git_repo)?;
